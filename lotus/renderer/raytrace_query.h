...
public:
  ...
  Task<float> query(ObjectFlags object_flags, glm::vec3 origin,
                    glm::vec3 direction, float min, float max);

private:
  class RaytraceQuery {
  public:
    RaytraceQuery(ObjectFlags _object_flags, glm::vec3 _origin,
                  glm::vec3 _direction, float _min, float _max)
        : object_flags(_object_flags), origin(_origin), direction(_direction),
          min(_min), max(_max) {}
  ...
  Task<float> query_queue(ObjectFlags object_flags, glm::vec3 origin,
                          glm::vec3 direction, float min, float max);
  ...
  vk::UniqueHandle<vk::DescriptorSetLayout, vk::DispatchLoaderDynamic>
      rtx_descriptor_layout;
  vk::UniqueHandle<vk::DescriptorPool, vk::DispatchLoaderDynamic>
      rtx_descriptor_pool;
  vk::UniqueHandle<vk::DescriptorSet, vk::DispatchLoaderDynamic>
      rtx_descriptor_set;
