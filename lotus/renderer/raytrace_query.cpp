...
RaytraceQueryer::RaytraceQueryer(Engine* _engine) : engine(_engine)
{
    if (engine->config->renderer.RaytraceEnabled())
    {
        ...
        vk::DeviceSize shader_offset_miss =
            (((shader_stride * shader_raygencount) / engine->renderer->gpu->ray_tracing_properties.shaderGroupBaseAlignment) + 1) *
            engine->renderer->gpu->ray_tracing_properties.shaderGroupBaseAlignment;
        vk::DeviceSize shader_offset_hit =
            shader_offset_miss + (((shader_stride * shader_misscount) / engine->renderer->gpu->ray_tracing_properties.shaderGroupBaseAlignment) + 1) *
                                     engine->renderer->gpu->ray_tracing_properties.shaderGroupBaseAlignment;
        vk::DeviceSize sbt_size = (shader_stride * shader_hitcount) + shader_offset_hit;
        shader_binding_table = engine->renderer->gpu->memory_manager->GetAlignedBuffer(
            sbt_size, engine->renderer->gpu->ray_tracing_properties.shaderGroupBaseAlignment,
        ...
        std::vector<vk::DescriptorSetLayoutBinding> rtx_bindings{acceleration_structure_binding, input_ubo_binding, output_buffer_binding};
        ...
        engine->renderer->gpu->device->getRayTracingShaderGroupHandlesKHR(*rtx_pipeline, 0, shader_nonhitcount + shader_hitcount, shader_handle_storage.size(),
        ...
        {
            memcpy(shader_mapped + shader_offset_miss + (i * shader_stride),
                   shader_handle_storage.data() + (shader_stride * shader_raygencount) + (i * shader_stride), shader_stride);
        }
        for (uint32_t i = 0; i < shader_hitcount; ++i)
        {
            memcpy(shader_mapped + shader_offset_hit + (i * shader_stride),
                   shader_handle_storage.data() + (shader_stride * shader_nonhitcount) + (i * shader_stride), shader_stride);
        }
        shader_binding_table->unmap();

        raygenSBT =
            vk::StridedDeviceAddressRegionKHR{engine->renderer->gpu->device->getBufferAddress({.buffer = shader_binding_table->buffer}) + shader_offset_raygen,
                                              shader_stride, shader_stride * shader_raygencount};
        missSBT =
            vk::StridedDeviceAddressRegionKHR{engine->renderer->gpu->device->getBufferAddress({.buffer = shader_binding_table->buffer}) + shader_offset_miss,
                                              shader_stride, shader_stride * shader_misscount};
        hitSBT =
            vk::StridedDeviceAddressRegionKHR{engine->renderer->gpu->device->getBufferAddress({.buffer = shader_binding_table->buffer}) + shader_offset_hit,
                                              shader_stride, shader_stride * shader_hitcount};
    }
    ...
    if (engine->config->renderer.RaytraceEnabled())
    {
        ...
        if (engine->game->scene && tlas && tlas->handle != 0)
        {
            ...
            while (local_task_count > 0)
            {
                ...
                input_buffer =
                    engine->renderer->gpu->memory_manager->GetBuffer(input_buffer_size, vk::BufferUsageFlagBits::eUniformBuffer,
                                                                     vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent);
                output_buffer =
                    engine->renderer->gpu->memory_manager->GetBuffer(output_buffer_size, vk::BufferUsageFlagBits::eStorageBuffer,
                                                                     vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent);
