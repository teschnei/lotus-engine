...
class Memory {
public:
  Memory(MemoryManager *_manager, VmaAllocation _allocation,
         VmaAllocationInfo _alloc_info, vk::DeviceSize _size)
      : allocation(_allocation), memory_offset(_alloc_info.offset),
        memory(_alloc_info.deviceMemory), size(_size), manager(_manager) {}
...
public:
  Buffer(MemoryManager *_manager, vk::Buffer _buffer, VmaAllocation _allocation,
         VmaAllocationInfo _alloc_info, vk::DeviceSize _size)
...
public:
  Image(MemoryManager *_manager, vk::Image _image, VmaAllocation _allocation,
        VmaAllocationInfo _alloc_info, vk::DeviceSize _size)
...
public:
  GenericMemory(MemoryManager *_manager, VmaAllocation _allocation,
                VmaAllocationInfo _alloc_info, vk::DeviceSize _size)
...
public:
  ...
  ~MemoryManager();
  std::unique_ptr<Buffer> GetBuffer(vk::DeviceSize size,
                                    vk::BufferUsageFlags usage,
                                    vk::MemoryPropertyFlags memoryflags);
  std::unique_ptr<Buffer> GetAlignedBuffer(vk::DeviceSize size,
                                           vk::DeviceSize alignment,
                                           vk::BufferUsageFlags usage,
                                           vk::MemoryPropertyFlags memoryflags);
  std::unique_ptr<Image> GetImage(uint32_t width, uint32_t height,
                                  vk::Format format, vk::ImageTiling tiling,
                                  vk::ImageUsageFlags usage,
                                  vk::MemoryPropertyFlags memoryflags,
                                  uint32_t arrayLayers = 1);
  std::unique_ptr<GenericMemory> GetMemory(
