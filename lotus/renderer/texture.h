...
public:
  ...
  static Task<std::shared_ptr<Texture>> LoadTexture(std::string texturename,
                                                    Func func, Args &&...args) {
    if (auto found = texture_map.find(texturename);
        found != texture_map.end()) {
  ...
  WorkerTask<> Init(Engine *engine,
                    std::vector<std::vector<uint8_t>> &&texture_data);
...
protected:
  ...
  inline static std::unordered_map<std::string, std::weak_ptr<Texture>>
      texture_map{};
};
