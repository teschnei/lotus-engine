...
Renderer::~Renderer() { gpu->device->waitIdle(); }
...
void Renderer::createInstance(const std::string& app_name, uint32_t app_version)
{
    ...
    if (enableValidationLayers)
    {
        ...
        debugCreateInfo.messageSeverity = vk::DebugUtilsMessageSeverityFlagBitsEXT::eVerbose | vk::DebugUtilsMessageSeverityFlagBitsEXT::eWarning |
...
void Renderer::createSwapchain() { swapchain = std::make_unique<Swapchain>(engine->config.get(), gpu.get(), window.get(), *surface); }
...
void Renderer::createCommandPool() { command_pool = gpu->createCommandPool(GPU::QueueType::Graphics, vk::CommandPoolCreateFlagBits::eResetCommandBuffer); }
...
Task<> Renderer::recreateStaticCommandBuffers()
{
    ...
    //TODO: generalize this? only baked commands need to be cleared here, and either no commands should be baked or just
    landscape should be engine->game->scene->forEachEntity([](std::shared_ptr<Entity>& entity)
    {
...
void Renderer::createDeferredImage()
{
    ...
    deferred_image = gpu->memory_manager->GetImage(swapchain->extent.width, swapchain->extent.height, format, vk::ImageTiling::eOptimal,
...
vk::CommandBuffer Renderer::prepareDeferredImageForPresent()
{
    ...
    std::array image_blits{vk::ImageBlit{.srcSubresource = {.aspectMask = vk::ImageAspectFlagBits::eColor, .mipLevel = 0, .baseArrayLayer = 0, .layerCount = 1},
