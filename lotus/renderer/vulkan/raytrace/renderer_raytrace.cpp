...
void RendererRaytrace::createDescriptorSetLayout()
{
    ...
    std::vector<vk::DescriptorSetLayoutBinding> deferred_bindings = {albedo_sample_layout_binding, light_sample_layout_binding,
...
void RendererRaytrace::createRaytracingPipeline()
{
    ...
    raytracer = std::make_unique<RaytracePipeline>(this, "raytrace_pure", descriptors);
}
...
void RendererRaytrace::createGraphicsPipeline()
{
    {
        // deferred pipeline
        auto shader_module = getShader("shaders/deferred_raytrace.spv");
...
void RendererRaytrace::createGBufferResources()
{
    gbuffer.albedo.image =
        gpu->memory_manager->GetImage(swapchain->extent.width, swapchain->extent.height, vk::Format::eR32G32B32A32Sfloat, vk::ImageTiling::eOptimal,
                                      vk::ImageUsageFlagBits::eSampled | vk::ImageUsageFlagBits::eStorage, vk::MemoryPropertyFlagBits::eDeviceLocal);
    gbuffer.light.image =
        gpu->memory_manager->GetImage(swapchain->extent.width, swapchain->extent.height, vk::Format::eR32G32B32A32Sfloat, vk::ImageTiling::eOptimal,
                                      vk::ImageUsageFlagBits::eSampled | vk::ImageUsageFlagBits::eStorage, vk::MemoryPropertyFlagBits::eDeviceLocal);
    gbuffer.normal.image =
        gpu->memory_manager->GetImage(swapchain->extent.width, swapchain->extent.height, vk::Format::eR32G32B32A32Sfloat, vk::ImageTiling::eOptimal,
                                      vk::ImageUsageFlagBits::eSampled | vk::ImageUsageFlagBits::eStorage, vk::MemoryPropertyFlagBits::eDeviceLocal);
    gbuffer.particle.image =
        gpu->memory_manager->GetImage(swapchain->extent.width, swapchain->extent.height, vk::Format::eR32G32B32A32Sfloat, vk::ImageTiling::eOptimal,
                                      vk::ImageUsageFlagBits::eSampled | vk::ImageUsageFlagBits::eStorage, vk::MemoryPropertyFlagBits::eDeviceLocal);
    gbuffer.motion_vector.image =
        gpu->memory_manager->GetImage(swapchain->extent.width, swapchain->extent.height, vk::Format::eR32G32B32A32Sfloat, vk::ImageTiling::eOptimal,
                                      vk::ImageUsageFlagBits::eStorage, vk::MemoryPropertyFlagBits::eDeviceLocal);
...
vk::UniqueCommandBuffer RendererRaytrace::getDeferredCommandBuffer()
{
    ...
    std::array camera_buffer_info{vk::DescriptorBufferInfo{.buffer = camera_buffers.view_proj_ubo->buffer,
                                  ...
                                  vk::DescriptorBufferInfo{.buffer = camera_buffers.view_proj_ubo->buffer,
...
void RendererRaytrace::initializeCameraBuffers()
{
    camera_buffers.view_proj_ubo = engine->renderer->gpu->memory_manager->GetBuffer(
        engine->renderer->uniform_buffer_align_up(sizeof(Component::CameraComponent::CameraData)) * engine->renderer->getFrameCount(),
        vk::BufferUsageFlagBits::eUniformBuffer, vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent);
    camera_buffers.view_proj_mapped = static_cast<Component::CameraComponent::CameraData*>(camera_buffers.view_proj_ubo->map(
        0, engine->renderer->uniform_buffer_align_up(sizeof(Component::CameraComponent::CameraData)) * engine->renderer->getFrameCount(), {}));
}
...
vk::CommandBuffer RendererRaytrace::getRenderCommandbuffer()
{
    ...
    vk::DescriptorImageInfo target_image_info_albedo{.imageView = *gbuffer.albedo.image_view, .imageLayout = vk::ImageLayout::eGeneral};

    vk::DescriptorImageInfo target_image_info_normal{.imageView = *gbuffer.normal.image_view, .imageLayout = vk::ImageLayout::eGeneral};

    vk::DescriptorImageInfo target_image_info_light{.imageView = *gbuffer.light.image_view, .imageLayout = vk::ImageLayout::eGeneral};

    vk::DescriptorImageInfo target_image_info_particle{.imageView = *gbuffer.particle.image_view, .imageLayout = vk::ImageLayout::eGeneral};

    vk::DescriptorImageInfo target_image_info_motion_vector{.imageView = *gbuffer.motion_vector.image_view, .imageLayout = vk::ImageLayout::eGeneral};

    std::array cam_buffer_info{vk::DescriptorBufferInfo{.buffer = camera_buffers.view_proj_ubo->buffer,
    ...
    vk::DescriptorBufferInfo light_buffer_info_global{.buffer = engine->lights->light_buffer->buffer,
                                                      .offset = getCurrentFrame() * engine->lights->GetBufferSize(),
                                                      .range = engine->lights->GetBufferSize()};

    std::array descriptors{vk::WriteDescriptorSet{
                               // albedo
                               .dstBinding = 0,
                               .dstArrayElement = 0,
                               .descriptorCount = 1,
...
Task<> RendererRaytrace::drawFrame()
{
    ...
    buffers.push_back({.commandBuffer = getRenderCommandbuffer()});
    // post process
    ...
    // std::ranges::transform(ui_buffers, deferred_buffers.begin() + 1, [](auto buffer) { return
    // vk::CommandBufferSubmitInfoKHR{ .commandBuffer = buffer }; });
    ...
    frame_ready_value = timeline_sem_base[current_frame] + timeline_frame_ready;
