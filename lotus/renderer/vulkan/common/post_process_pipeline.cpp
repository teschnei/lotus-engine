...
void PostProcessPipeline::Init()
{
    ...
    std::array descriptors{// input colour
    ...
    // pipeline layout
    vk::PipelineLayoutCreateInfo pipeline_layout_ci{.setLayoutCount = static_cast<uint32_t>(layouts.size()), .pSetLayouts = layouts.data()};
    ...
    for (auto& buffer : image_buffers)
    {
        buffer.image = renderer->gpu->memory_manager->GetImage(
            renderer->swapchain->extent.width, renderer->swapchain->extent.height, vk::Format::eR32G32B32A32Sfloat, vk::ImageTiling::eOptimal,
    ...
    history_sampler = renderer->gpu->device->createSamplerUnique({.magFilter = vk::Filter::eLinear,
    ...
    factor_sampler = renderer->gpu->device->createSamplerUnique({.magFilter = vk::Filter::eNearest,
...
vk::UniqueCommandBuffer PostProcessPipeline::getCommandBuffer(vk::ImageView input_colour, vk::ImageView input_normals, vk::ImageView input_motionvectors)
{
    ...
    vk::DescriptorImageInfo input_colour_info{.imageView = input_colour, .imageLayout = vk::ImageLayout::eGeneral};

    vk::DescriptorImageInfo input_normal_info{.imageView = input_normals, .imageLayout = vk::ImageLayout::eGeneral};

    vk::DescriptorImageInfo input_motionvector_info{.imageView = input_motionvectors, .imageLayout = vk::ImageLayout::eGeneral};

    vk::DescriptorImageInfo input_factor_info{
        .sampler = *factor_sampler, .imageView = *factor_buffers[buffer_index].image_view, .imageLayout = vk::ImageLayout::eShaderReadOnlyOptimal};
    ...
    vk::DescriptorImageInfo output_factor_info{.imageView = *factor_buffers[buffer_index].image_view, .imageLayout = vk::ImageLayout::eGeneral};

    vk::DescriptorImageInfo output_image_info{.imageView = *image_buffers[buffer_index].image_view, .imageLayout = vk::ImageLayout::eGeneral};
...
vk::ImageView PostProcessPipeline::getOutputImageView() { return *image_buffers[buffer_index].image_view; }
} // namespace lotus
