...
WorkerTask<> RendererHybrid::InitWork()
{
    ...
    std::vector barriers = {vk::ImageMemoryBarrier2KHR{
...
void RendererHybrid::createDescriptorSetLayout()
{
    ...
    std::vector<vk::DescriptorSetLayoutBinding> rtx_deferred_bindings = {
        position_sample_layout_binding, albedo_sample_layout_binding,       light_sample_layout_binding,
        material_index_layout_binding,  accumulation_sample_layout_binding, revealage_sample_layout_binding,
        light_buffer_binding,           light_type_sample_layout_binding,   particle_sample_layout_binding,
...
{
    std::array descriptors{vk::DescriptorSetLayoutBinding{// light output
                                                          .binding = 0,
...
vk::UniqueCommandBuffer RendererHybrid::getDeferredCommandBuffer()
{
    ...
    std::array pre_render_transitions{vk::ImageMemoryBarrier2KHR{
...
std::vector<vk::CommandBuffer> RendererHybrid::getRenderCommandbuffers()
{
    ...
    vk::DescriptorImageInfo target_image_info_colour{.imageView = *rtx_gbuffer.colour.image_view, .imageLayout = vk::ImageLayout::eGeneral};

    vk::DescriptorBufferInfo light_buffer_info{.buffer = engine->lights->light_buffer->buffer,
                                               .offset = getCurrentFrame() * engine->lights->GetBufferSize(),
                                               .range = engine->lights->GetBufferSize()};

    vk::DescriptorImageInfo position_info{.sampler = *rasterizer->getGBuffer().sampler,
                                          .imageView = *rasterizer->getGBuffer().position.image_view,
                                          .imageLayout = vk::ImageLayout::eShaderReadOnlyOptimal};

    vk::DescriptorImageInfo normal_info{.sampler = *rasterizer->getGBuffer().sampler,
                                        .imageView = *rasterizer->getGBuffer().normal.image_view,
                                        .imageLayout = vk::ImageLayout::eShaderReadOnlyOptimal};

    vk::DescriptorImageInfo face_normal_info{.sampler = *rasterizer->getGBuffer().sampler,
                                             .imageView = *rasterizer->getGBuffer().face_normal.image_view,
                                             .imageLayout = vk::ImageLayout::eShaderReadOnlyOptimal};

    vk::DescriptorImageInfo albedo_info{.sampler = *rasterizer->getGBuffer().sampler,
                                        .imageView = *rasterizer->getGBuffer().albedo.image_view,
                                        .imageLayout = vk::ImageLayout::eShaderReadOnlyOptimal};

    vk::DescriptorImageInfo material_index_info{.sampler = *rasterizer->getGBuffer().sampler,
    ...
    std::array after_writes{vk::ImageMemoryBarrier2KHR{
...
Task<> RendererHybrid::drawFrame()
{
    ...
    // post process
    auto post_buffer = post_process->getCommandBuffer(*rtx_gbuffer.colour.image_view, *rasterizer->getGBuffer().normal.image_view,
                                                      *rasterizer->getGBuffer().motion_vector.image_view);
