...
module lotus:renderer.vulkan.buffer_resource;
...
namespace lotus
{
template <typename T, uint32_t _Binding> class BufferResource
{
public:
    class View
    {
    public:
        View(BufferResource* _resource, uint32_t _count)
            : resource(_resource), count(_count), index(_resource->get_index(count)), buffer_view(resource->buffer_mapped + index, count)
        {
        }
    ...
    BufferResource(MemoryManager* memory_manager, vk::Device device, vk::DescriptorSet _set)
    {
        ...
        vk::DescriptorBufferInfo write_buffer{.buffer = this->buffer->buffer, .offset = 0, .range = size};
        vk::WriteDescriptorSet write{
            .dstSet = _set, .dstBinding = _Binding, .dstArrayElement = 0, .descriptorCount = 1, .descriptorType = Type, .pBufferInfo = &write_buffer};
        device.updateDescriptorSets(write, nullptr);
    }
    ~BufferResource()
    {
        if (this->buffer_mapped)
            this->buffer->unmap();
    }

    std::unique_ptr<View> get(uint32_t count) { return std::make_unique<View>(this, count); }
...
private:
    ...
    void free_index(uint32_t index, uint32_t count) { free_indices[count].queue(index); }
