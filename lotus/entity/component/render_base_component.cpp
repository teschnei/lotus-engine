...
{
    RenderBaseComponent::RenderBaseComponent(Entity* _entity, Engine* _engine) :
    ...
    Task<> RenderBaseComponent::init()
    {
        ...
        uniform_buffer_mapped = static_cast<uint8_t*>(uniform_buffer->map(0, engine->renderer->uniform_buffer_align_up(sizeof(UniformBufferObject)) * engine->renderer->getFrameCount(), {}));
        co_return;
    }
    ...
    Task<> RenderBaseComponent::tick(time_point time, duration elapsed)
    {
        ...
        UniformBufferObject* ubo = reinterpret_cast<UniformBufferObject*>(uniform_buffer_mapped + engine->renderer->getCurrentFrame() * engine->renderer->uniform_buffer_align_up(sizeof(UniformBufferObject)));
        ubo->model = model;
        ubo->modelIT = modelIT;
