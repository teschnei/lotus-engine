...
export module lotus:util.task;

namespace lotus
{
// this works when co_await returns the awaiter, but may need something for any other case
template <typename T> using AwaiterType = decltype(std::declval<T>().operator co_await());

template <typename T> using AwaitableReturn = decltype(std::declval<AwaiterType<T>>().await_resume());
...
export template <typename Result> struct Task;
...
export template <typename Result> struct Promise : public Promise_base
{
    using coroutine_handle = std::coroutine_handle<Promise<Result>>;
    Task<Result> get_return_object() noexcept;

    template <typename R>
        requires std::convertible_to<R&&, Result>
...
export template <> struct Promise<void> : public Promise_base
{
...
export template <typename Result = void> struct [[nodiscard]] Task
{
...
template <typename Result> Task<Result> Promise<Result>::get_return_object() noexcept { return Task<Result>{coroutine_handle::from_promise(*this)}; }

inline Task<void> Promise<void>::get_return_object() noexcept { return Task<void>{coroutine_handle::from_promise(*this)}; }
}; // namespace lotus
