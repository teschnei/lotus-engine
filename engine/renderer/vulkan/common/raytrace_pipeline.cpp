...
namespace lotus
{
    RaytracePipeline::RaytracePipeline(Renderer* _renderer, std::string raygen, std::span<vk::DescriptorSetLayoutBinding> input_output_descriptor_layout_desc) : renderer(_renderer)
    {
    ...
    vk::UniqueDescriptorSetLayout RaytracePipeline::initializeResourceDescriptorSetLayout(Renderer* renderer)
    {
        std::array descriptors {
            vk::DescriptorSetLayoutBinding { //acceleration structure
                .binding = 0,
    ...
    vk::UniqueDescriptorSetLayout RaytracePipeline::initializeInputOutputDescriptorSetLayout(Renderer* renderer, std::span<vk::DescriptorSetLayoutBinding> descriptors)
    {
        return renderer->gpu->device->createDescriptorSetLayoutUnique({
    ...
    vk::UniquePipeline RaytracePipeline::initializePipeline(Renderer* renderer, vk::PipelineLayout pipeline_layout, std::string raygen)
    {
        ...
        std::array shaders {
            vk::PipelineShaderStageCreateInfo { //raygen
        ...
        return renderer->gpu->device->createRayTracingPipelineKHRUnique(nullptr, nullptr, {
    ...
    RaytracePipeline::SBT RaytracePipeline::initializeSBT(Renderer* renderer, vk::Pipeline pipeline)
    {
        ...
        renderer->gpu->device->getRayTracingShaderGroupHandlesKHR(pipeline, 0, shader_nonhitcount + shader_hitcount, shader_handle_storage.size(), shader_handle_storage.data());
        ...
        return {
            .buffer = std::move(shader_binding_table),
            .raygen = {
                .deviceAddress = renderer->gpu->device->getBufferAddress({.buffer = buffer}) + shader_offset_raygen,
                .stride = nonhit_shader_stride,
                .size = nonhit_shader_stride * shader_raygencount
            },
            .miss = {
                .deviceAddress = renderer->gpu->device->getBufferAddress({.buffer = buffer}) + shader_offset_miss,
                .stride = nonhit_shader_stride,
                .size = nonhit_shader_stride * shader_misscount
            },
            .hit = {
                .deviceAddress = renderer->gpu->device->getBufferAddress({.buffer = buffer}) + shader_offset_hit,
        ...
        std::span<vk::ImageMemoryBarrier2KHR> before_barriers, std::span<vk::ImageMemoryBarrier2KHR> after_barriers)
    {
        ...
        {
            buffer[0]->pipelineBarrier2KHR({
                .imageMemoryBarrierCount = static_cast<uint32_t>(after_barriers.size()),
