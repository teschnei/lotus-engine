...
namespace lotus
{
    ...
    void TopLevelAccelerationStructure::AddBLASResource(Model* model)
    {
    ...
    void TopLevelAccelerationStructure::AddBLASResource(DeformableEntity* entity)
    {
        uint32_t image = renderer->getCurrentImage();
    ...
    void TopLevelAccelerationStructure::AddBLASResource(Particle* entity)
    {
        uint32_t image = renderer->getCurrentImage();
        uint16_t index = static_cast<uint16_t>(descriptor_vertex_info.size()) + renderer->static_acceleration_bindings_offset;
